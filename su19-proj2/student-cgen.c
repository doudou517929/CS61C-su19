...
void ProcessExprBinaryAdd(DAST* dast,
  ...
  } else if (c2PtrTotal) { // Pointer arithmetic case 1
...
void ProcessExprBinarySub(DAST* dast,
  ...
  MatchSizes (child1, firstReg, child2, S1);
  /* STUDENT CODE HERE. */
  if (c1PtrTotal && !c2PtrTotal) { // eg.x - 1
      int size = -1 * child1->typeDecl->dataSize;
      EmitADDI(T0, X0, size);
      EmitMUL(S1, S1, T0);
      EmitADD(S1, firstReg, S1);
  } else if (c2PtrTotal && !c1PtrTotal) {
      // make no sense. eg.1 - x
  } else if (c1PtrTotal && c2PtrTotal) {
      // Potential erroneous code
      int size = child1->typeDecl->dataSize;
      EmitADDI(T0, X0, size);
      EmitMUL(S1, S1, T0);
      EmitADD(firstReg, firstReg, S1);
      EmitADD(S1, firstReg, S1);
      EmitDIV(S1, S1, T0);
  } else {
...
void ProcessExprBinaryGTEq(DAST* dast,
  ...
  MatchSizes (child1, firstReg, child2, S1);
  /* STUDENT CODE HERE. */
  if (c1PtrTotal && c2PtrTotal) { // pointer comparison
